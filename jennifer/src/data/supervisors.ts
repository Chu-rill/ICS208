...
export const generateSupervisorStudents = (
  supervisorId: string,
  count: number = 6
): SupervisorStudent[] => {
    ...
    const student: SupervisorStudent = {
      ...
      companyName:
        i % 3 === 0
          ? "TechNigeria Ltd"
          : i % 3 === 1
          ? "Nigerian Engineering Solutions"
          : "Global Oil Nigeria",
      position:
        i % 4 === 0
...
export const generateAssessments = (
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const assessment: Assessment = {
      ...
      status,
      createdAt: format(
        sub(new Date(), { weeks: i, days: 1 }),
        "yyyy-MM-dd'T'HH:mm:ss'Z'"
      ),
      updatedAt: format(
        sub(new Date(), { weeks: i }),
        "yyyy-MM-dd'T'HH:mm:ss'Z'"
      ),
    };
...
export const generateVisitRecords = (
  supervisorId: string,
  studentId: string
): VisitRecord[] => {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const visit: VisitRecord = {
      ...
      purpose:
        i === 0
          ? "Introductory visit and workplace assessment"
          : "Progress evaluation and feedback session",
  ...
  const upcomingVisit: VisitRecord = {
    ...
    notes:
      "Will review current project contributions and provide feedback on areas for improvement.",
    followUpActions: [
  ...
  const cancelledVisit: VisitRecord = {
    ...
    notes:
      "Cancelled due to unexpected supervisor meeting. Rescheduled for next week.",
  };
