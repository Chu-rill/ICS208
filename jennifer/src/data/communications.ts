...
export const generateNotifications = (userId: string): Notification[] => {
  ...
  notifications.push({
    ...
    message: 'A new announcement about SIWES guidelines has been posted. Please review it as soon as possible.',
    type: 'info',
    read: false,
    ...
    switch (user.role) {
      case 'student':
        notifications.push({
          ...
          message: 'Your supervisor has reviewed your latest logbook entry. Please check their comments.',
          type: 'success',
          read: false,
          actionLink: '/logbook',
          createdAt: format(sub(currentDate, { hours: 6 }), 'yyyy-MM-dd\'T\'HH:mm:ss\'Z\'')
        });
        ...
        notifications.push({
          ...
          message: 'Your monthly assessment is scheduled for next week. Please prepare all required documentation.',
          type: 'warning',
          read: false,
        ...
        notifications.push({
          ...
          message: 'You have not submitted a logbook entry for the past 2 days. Please update your logbook.',
          type: 'error',
          read: false,
      ...
      case 'industry_supervisor':
        notifications.push({
          ...
          message: 'You have 3 logbook entries pending review. Please complete them at your earliest convenience.',
          type: 'warning',
          read: false,
          actionLink: '/supervisor/reviews',
          createdAt: format(sub(currentDate, { hours: 4 }), 'yyyy-MM-dd\'T\'HH:mm:ss\'Z\'')
        });
        ...
        notifications.push({
          ...
          message: 'A new student has been assigned to you for supervision. Please review their details.',
          type: 'info',
          read: true,
        ...
        if (user.role === 'academic_supervisor') {
          notifications.push({
            ...
            message: 'You have a scheduled visitation tomorrow for student Adebayo Johnson at TechNigeria Ltd.',
            type: 'warning',
            read: false,
            actionLink: '/supervisor/visits',
            createdAt: format(sub(currentDate, { hours: 24 }), 'yyyy-MM-dd\'T\'HH:mm:ss\'Z\'')
          });
        }
        break;
      ...
      case 'coordinator':
        notifications.push({
          ...
          message: 'The monthly SIWES reports are now available for review and distribution.',
          type: 'info',
          read: false,
          actionLink: '/coordinator/reports',
          createdAt: format(sub(currentDate, { hours: 2 }), 'yyyy-MM-dd\'T\'HH:mm:ss\'Z\'')
        });
        ...
        notifications.push({
          ...
          message: 'There are 5 students without assigned supervisors. Please make the necessary assignments.',
          type: 'warning',
          read: false,
        ...
        notifications.push({
          ...
          message: 'Some supervisors have low engagement metrics. Consider scheduling a review meeting.',
          type: 'error',
          read: true,
      ...
      case 'administrator':
        notifications.push({
          ...
          message: 'A new system update is available. Please schedule installation at your convenience.',
          type: 'info',
          read: false,
          actionLink: '/admin/settings',
          createdAt: format(sub(currentDate, { hours: 8 }), 'yyyy-MM-dd\'T\'HH:mm:ss\'Z\'')
        });
        ...
        notifications.push({
          ...
          message: 'A new organization has registered for the SIWES program. Please review and approve.',
          type: 'warning',
          read: false,
        ...
        notifications.push({
          ...
          message: 'Multiple users are reporting login issues. Please investigate the authentication system.',
          type: 'error',
          read: true,
...
export const generateConversations = (userId: string): Conversation[] => {
  ...
  const numConversations = Math.min(3 + Math.floor(Math.random() * 3), potentialPartners.length);
  ...
  for (let i = 0; i < numConversations; i++) {
    ...
    if (partner) {
      ...
      const unreadCount = Math.random() > 0.5 ? Math.floor(Math.random() * 3) : 0;
      ...
      conversations.push({
        ...
        lastMessage: {
          ...
          createdAt: format(sub(currentDate, { hours: timeAgo }), 'yyyy-MM-dd\'T\'HH:mm:ss\'Z\'')
        },
        unreadCount
      });
    }
...
export const generateMessages = (conversationId: string, userId: string): Message[] => {
  ...
  for (let i = 0; i < numMessages; i++) {
    ...
    const message: Message = {
      ...
      createdAt: format(sub(currentDate, { hours: hoursAgo }), 'yyyy-MM-dd\'T\'HH:mm:ss\'Z\'')
    };
...
function getRandomMessage(senderRole: string, recipientRole: string): string {
  ...
  if (senderRole === 'student' && (recipientRole.includes('supervisor') || recipientRole === 'coordinator')) {
    return studentToSupervisorMessages[Math.floor(Math.random() * studentToSupervisorMessages.length)];
  } else if ((senderRole.includes('supervisor')) && recipientRole === 'student') {
    return supervisorToStudentMessages[Math.floor(Math.random() * supervisorToStudentMessages.length)];
  } else if (senderRole === 'coordinator') {
    return coordinatorMessages[Math.floor(Math.random() * coordinatorMessages.length)];
