...
export const generateLogbookEntries = (
  studentId: string,
  count: number = 20
): LogbookEntry[] => {
  ...
  return {
    ...
    companyName:
      studentId === "1"
        ...
        ? "Global Oil Nigeria"
        : "Datamax Systems",
    companyLogo: `https://ui-avatars.com/api/?name=${
      studentId === "1"
        ? "Tech+Nigeria"
        : studentId === "2"
  ...
  return {
    ...
    completedDays:
      studentId === "1"
        ...
        : 75,
    pendingApprovals:
      studentId === "1" ? 2 : studentId === "2" ? 3 : studentId === "3" ? 1 : 4,
    totalEntries:
      studentId === "1"
        ? 105
        : studentId === "2"
...
function getRandomComment(status: string): string {
  if (status === "approved") {
    ...
    return approvalComments[
      Math.floor(Math.random() * approvalComments.length)
    ];
